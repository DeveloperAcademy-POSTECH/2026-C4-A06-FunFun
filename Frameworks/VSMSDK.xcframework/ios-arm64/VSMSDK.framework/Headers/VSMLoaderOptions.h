...
typedef NS_OPTIONS(UInt32, VSMLoaderOptionsFlags) {
    ...
    /**
     * Configuration 및 리소스 다운로드 실패 시 로컬에 저장된 데이터를 사용합니다.
     */
    LOADER_FLAG_USE_FALLBACK        = 1 << 10,
    
    /**
     * Resource 다운로드 실패 시 재시도 하지 않습니다.
     */
    LOADER_FLAG_RESOURCE_NO_RETRY  = 1 << 11,
    /**
     * 다운로드 진행 callback을 전달하지 않습니다.
     */
    LOADER_FLAG_NO_PROGRESS        = 1 << 12
};
