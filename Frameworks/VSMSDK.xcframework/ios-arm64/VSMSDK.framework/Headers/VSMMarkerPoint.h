...
/** 텍스트 앵커 - 디폴트:(0.5, 0.5)
 *  텍스트 내부 영역중 position좌표와 일치되는 정규화된 좌표. 좌상단은 (0, 0)이고 우상단은 (1, 1)입니다.
 */
@property (nonatomic, assign) CGPoint textAnchor;
...
/** texts
 *  text 와 text 스타일을 설정합니다.
 */
@property (nonatomic, copy, nullable) NSArray<VSMMarkerText *> *texts;

@property (nonatomic, assign) BOOL bShowMarkerBBox;

/** marker 겹침 영역 픽셀 (LTRB) - 디폴트 (0,0,0,0)
 *   현재 아이콘 사이즈 기준으로 겹침 영역을 각각 왼쪽, 오른쪽, 위쪽, 아래쪽으로 얼마나 늘려야 할지 설정합니다.
 */
@property (nonatomic, assign) UIEdgeInsets overlapPadding;

@property (nonatomic, copy)   NSString *text
API_DEPRECATED_WITH_REPLACEMENT("texts", ios(9.0, 17.0));
 ...
 *  text 와 text 스타일을 설정합니다.
 */
@property (nonatomic, copy) NSArray<VSMMarkerText *> *texts;

/** marker 겹침 영역 Box 표출 유무
 */
@property (nonatomic, assign) BOOL bShowMarkerBBox;
