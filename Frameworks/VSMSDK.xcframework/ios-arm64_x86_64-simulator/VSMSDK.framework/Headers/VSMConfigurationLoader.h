...
/**
 * 초기화 데이터 다운로드 진행 상태를 위한 callback
 *
 * @param phase 현재 단계(0: appconfig, 1: config, 2: resource)
 * @param totalPhase 전체 단계 개수
 * @param count 다운로드된 개수
 * @param total 다운로드할 전체 개수
 * @param downloadedSize 다운로드된 사이즈(Byte)
 * @param totalSize 다운로드할 전체 파일 사이즈(Byte). 사전 다운로드 사이즈를 계산할 수 없으면 0이 전달됩니다.
 */
-(void)onConfigurationLoaderProgress:(NSUInteger)phase
                          totalPhase:(NSUInteger)totalPhase
