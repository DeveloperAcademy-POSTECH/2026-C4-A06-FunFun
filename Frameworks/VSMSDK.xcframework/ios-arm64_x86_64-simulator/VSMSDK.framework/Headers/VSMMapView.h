...
@class VSMFeatureQueryResult;
@class VSMFeatureQueryOptions;
@protocol VSMFeatureQueryDelegate;
...
/** ScaleBar 출력 여부 (default: NO)
 */
@property (nonatomic, assign) bool showScaleBar;

/** BlockedRoads 출력 여부 (default: YES)
 */
@property (nonatomic, assign) bool showBlockedRoads;

/** BlockedIcons 출력 여부 (default: YES)
 */
@property (nonatomic, assign) bool showBlockedIcons;
...
/** Extend layer 표시 여부를 설정합니다.
 */
- (void)setExtendLayerVisibility:(VSMMapExtendLayerType)layerType visible:(BOOL)visible;

/** Extend layer 표시 여부를 반환합니다.
 */
- (BOOL)getExtendLayerVisibility:(VSMMapExtendLayerType)layerType;
...
/** Map Zoom 확인 */
@property (nonatomic, assign) float mapZoomFactor ;

/** pixel based tile size 확인 */
@property (nonatomic, assign) int tilePxSize;

/** 고정배율 렌더링 유무 확인 */
@property (nonatomic, assign) bool fixedScaleRendering;

/** Marker BBox 표시 유무 */
@property (nonatomic, assign) bool showMarkerBBox;
...
/** 타일 로드 로그 출력
 */
@property (nonatomic, assign) bool showTileLoadLog;
...
/** 세부 스타일을 추가
 @param style 세부 스타일명.
 @deprecated Use setStyleCondition:value: instead
 */
- (void)addSubStyle:(NSString*_Nonnull)style __attribute__((deprecated("Use setStyleCondition:value: instead")));

/** 세부 스타일을 제거
 @param style 세부 스타일명.
 @deprecated Use removeStyleCondition: instead
 */
- (void)removeSubStyle:(NSString*_Nonnull)style __attribute__((deprecated("Use removeStyleCondition: instead")));

/** 스타일 조건을 설정합니다. style config의 $$key 조건에 매칭됩니다.
 @param key 조건 키
 @param value 조건 값
 */
- (void)setStyleCondition:(NSString*_Nonnull)key value:(NSString*_Nonnull)value;

/** 설정된 스타일 조건을 제거합니다.
 @param key 조건 키
 */
- (void)removeStyleCondition:(NSString*_Nonnull)key;
...
/**
* Map Zoom
* scale 값을 입력받아 맵 전체를 확대합니다.
*- 예: scale = 1.2f → 모든 오브젝트(지명, 경로, 아이콘 등)가 1.2배 확대
 */
-(void)setMapZoomFactor:(float)mapZoomFactor;

/**
* Tile size based on pixel
* 정수 줌일 때 맵 타일의 크기를 고정 픽셀 단위로 설정합니다.
*- 예: 256 → 모든 타일이 화면에서 항상 256px 크기를 유지
 */
-(void)setTilePixelMode:(int)tilePxSize;
...
#pragma mark - Feature Query

/**
 * Feature 조회 델리게이트
 * 실시간 Feature 조회가 활성화된 경우 조회 결과가 이 델리게이트를 통해 전달됩니다.
 * @see VSMFeatureQueryDelegate
 */
@property (nonatomic, weak, nullable) id<VSMFeatureQueryDelegate> featureQueryDelegate;

/**
 * 지정된 Screen Rect 영역 내의 Feature들을 동기적으로 조회합니다.
 * 현재 보이는 타일의 ServiceLayer Feature만 대상으로 조회합니다.
 * Marker는 포함되지 않으며, 렌더 스레드에서만 호출 가능합니다.
 *
 * @param rect 조회 영역 (Screen 좌표계)
 * @param options 조회 옵션
 * @return 조회 결과, 실패 시 nil
 * @see VSMFeatureQueryOptions
 * @see VSMFeatureQueryResult
 */
- (nullable VSMFeatureQueryResult *)queryFeaturesInRect:(CGRect)rect
                                                options:(nonnull VSMFeatureQueryOptions *)options;

/**
 * Feature 조회 기능을 활성화/비활성화합니다.
 * 활성화 시 매 프레임 렌더링 후 delegate로 현재 보이는 타일의
 * Feature 정보를 전달합니다. 전체 화면 영역이 조회 대상이 됩니다.
 * Marker는 포함되지 않으며 (ServiceLayer Feature만 대상),
 * delegate는 렌더 스레드에서 호출됩니다.
 *
 * @param enabled 활성화 여부 (YES: 활성화, NO: 비활성화)
 * @param options 조회 옵션 (enabled=YES일 때만 사용, enabled=NO일 때 nil 가능)
 * @return 성공 시 YES, 실패 시 NO
 * @see VSMFeatureQueryOptions
 * @see VSMFeatureQueryDelegate
 */
- (BOOL)setFeatureQueryEnabled:(BOOL)enabled
                       options:(nullable VSMFeatureQueryOptions *)options;
